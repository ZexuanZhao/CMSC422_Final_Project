...
class NN:
    ...
    def compute_gradients(self, X, Y):
        ...
        for idx in range(num_layers-1):
            # Done 2: Calculate D (D_k in the tutorial) for forward pass (which is similar to self.predit).
        ...
        for idx in range(num_layers-2, -1, -1):
            # Done 3: Calculate grad_bs and grad_Ws, which are lists of gradients for b's and w's of each layer.
...
class SquaredLoss(LossFunction):
    def loss(self, Y, Yhat):
        ...
        in Yhat; compute the loss associated with these predictions.
        """
        # Done 0: loss function for squared loss.

        ### YOUR CODE HERE ###
        return np.power(Yhat - Y, 2).sum(axis = 0).mean()/2
    ...
    def lossGradient(self, Y, Yhat):
        ...
        Yhat; compute the gradient of the loss with respect to Yhat
        """
        #Done 1: gradient for squared loss.
...
class CELoss(LossFunction):
    def loss(self, Y, Yhat):
        ...
        in Yhat; compute the loss associated with these predictions.
        """
        #Done 4: loss function for cross-entropy loss.


        return xlogy(Y, Yhat) + xlogy(1 - Y, 1 - Yhat).sum()
    ...
    def lossGradient(self, Y, Yhat):
        ...
        """
        #Done 5: gradient for cross-entropy loss.
